...
typst c examples/combined.typ examples/combined.teco.pdf --font-path fonts/

magick -density 600 examples/combined.teco.pdf -background white -alpha off -resize 25% "test/page.png" # 150dpi -> 1240x1754 for A4
# magick -density 150 examples/combined.teco.pdf -background white -alpha off "test/page.png" # 150dpi -> 1240x1754 for A4
...
magick -size 3010x2139 canvas:none \
  ...
  \( test/page-1.png \( +clone -background black -shadow $SHADOW \) +swap -background none -layers merge +repage \) -geometry +800+350  -compose over -composite \
